...
export function formatNetworkError(error, url) {
  ...
  } else if (detectProxyEnv()) {
    lines.push(`Proxy env detected but not applied: ${detectProxyEnv()?.key}=${detectProxyEnv()?.value}`);
    lines.push('Suggestion: this should not happen; please report as a bug');
  } else {
